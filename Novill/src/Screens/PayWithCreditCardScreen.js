...
const PayWithCreditCardScreen=(props)=>{
    const {state,ordercredit,clearErrorMessage}=useContext(AuthContext);
...
<ImageBackground source={require("../Screens/images/img.jpeg")} style={{ width: '100%', height: '100%' }}>
 ...
 <Text style={styles.summaryText}>Total items:                                             {totalAmount}</Text>
      ...
      <View style={{ top:90 }}>
    <CartButton title="Order Now" onPress={()=>ordercredit({cart,address,totalAmount,totalPrice,user,pharm})}></CartButton>
 </View>
  </SafeAreaView>
</ImageBackground>
